...
exports.buildPracticalScoreList = buildPracticalScoreList;
/** 최하점 계산에서 제외할 키워드 */
...
function lookupDeductionLevel(studentScore, scoreTable) {
    if (!scoreTable || scoreTable.length === 0)
        return 0;
    const allScores = Array.from(new Set(scoreTable.map((l) => Number(l.배점)).filter((n) => !Number.isNaN(n))));
    allScores.sort((a, b) => b - a);
    const studentScoreNum = Number(studentScore);
    if (Number.isNaN(studentScoreNum))
        return 0;
...
function convertGradeToScore(grade, _U_ID, _eventName) {
